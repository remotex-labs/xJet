...
export class FrameworkService {
    ...
    constructor() {
        ...
        this.frameworkSourceMap = this.setSourceFile(this.filePath);
    ...
    setSource(source: string, path: string): SourceService {
        const key = normalize(path);
        if (this.sourceMaps.has(key))
            return this.sourceMaps.get(key)!;
    ...
    setSourceFile(path: string): SourceService {
        const key = normalize(path);
        const map = `${ path }.map`;
    ...
    private initializeSourceMap(source: string, key: string): SourceService {
